  ...
  const stepDetails: Record<ProgressIndicatorStep, StepDetails> = useMemo(
    () => ({
      [ConfirmModalState.WRAPPING]: {
        ...
        learnMoreLinkHref: showDeprecated ? SupportArticleURL.WETH_EXPLAINER : '',
      },
      [ConfirmModalState.RESETTING_TOKEN_ALLOWANCE]: {
        icon: <CurrencyLogo currency={trade?.inputAmount.currency} />,
        rippleColor: inputTokenColor,
      ...
      [ConfirmModalState.APPROVING_TOKEN]: {
        ...
        learnMoreLinkText: showDeprecated ? t`Why do I have to approve a token?` : '',
        learnMoreLinkHref: showDeprecated ? SupportArticleURL.APPROVALS_EXPLAINER : '',
      },
      // [ConfirmModalState.PERMITTING]: {
      //   icon: <Sign />,
      ...
      [ConfirmModalState.PENDING_CONFIRMATION]: {
        ...
        learnMoreLinkHref: showDeprecated
          ? isLimitTrade(trade)
