...
import { AllowanceState, SwapAllowance } from 'hooks/usePermit2Allowance'
...
import { isClassicTrade, isKyberswapTrade, isLimitTrade } from 'state/routing/utils'
    ...
    <>
      ...
      {showAcceptChanges ? (
        ...
        <AutoRow>
          ...
          >
            {isKyberswapTrade(trade) || isClassicTrade(trade) ? (
              <ThemedText.BodySmall color="neutral2" variant="secondary" pt={10} pb={15} pl={2}>
                {isKyberswapTrade(trade) || isClassicTrade(trade) ? 'This swap will be executed on ' : ''}
                {isKyberswapTrade(trade) ? 'Kyberswap' : isClassicTrade(trade) ? 'DeFi Pool' : ''}
              </ThemedText.BodySmall>
            ) : null}
