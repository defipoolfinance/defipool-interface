...
import { ClassicTrade, KyberswapTrade, ParaswapTrade, SubmittableTrade } from 'state/routing/types'
...
function useKyberswapGasPrice({ gasUseEstimateUSD }: KyberswapTrade) {
  const { formatNumber } = useFormatter()
  return gasUseEstimateUSD === 0 ? '<$0.01' : formatNumber({ input: gasUseEstimateUSD, type: NumberType.FiatGasPrice })
}

function useParaswapGasPrice({ gasUseEstimateUSD }: ParaswapTrade) {
  const { formatNumber } = useFormatter()
  return gasUseEstimateUSD === 0 ? '<$0.01' : formatNumber({ input: gasUseEstimateUSD, type: NumberType.FiatGasPrice })
}
...
export function SwapKyberswapRoute({ trade }: { trade: KyberswapTrade }) {
  const gasPrice = useKyberswapGasPrice(trade)

  // TODO: layout for Kyberswap route (copied from SwapRoute above )
  return (
    <Column gap="md">
      {/* <RoutingDiagram routes={routes} currencyIn={inputAmount.currency} currencyOut={outputAmount.currency} /> */}
      <ThemedText.Caption color="neutral2">
...
export function SwapParaswapRoute({ trade }: { trade: ParaswapTrade }) {
  const gasPrice = useParaswapGasPrice(trade)
