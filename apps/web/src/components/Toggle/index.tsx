...
const Wrapper = styled.button<{ isActive?: boolean; activeElement?: boolean }>`
  align-items: center;
  background: ${({ isActive, theme }) => (isActive ? theme.accent4 : 'transparent')};
...
const ToggleElementHoverStyle = (hasBgColor: boolean, theme: any, isActive?: boolean) =>
  hasBgColor
    ...
    : {
        background: isActive ? darken(0.05, theme.accent3) : darken(0.05, theme.surface3),
...
const ToggleElement = styled.span<{ isActive?: boolean; bgColor?: string; isInitialToggleLoad?: boolean }>`
  ...
  background: ${({ theme, bgColor, isActive }) =>
    isActive ? bgColor ?? theme.accent3 : bgColor ? theme.surface3 : theme.neutral3};
  border-radius: 50%;
  height: 24px;
  :hover {
