...
function VolumeChartSection({ chainId }: { chainId: number }) {
  ...
  if (isSmallScreen) {
    return (
      <MinimalStatDisplay
        title={<Trans>DeFi Pool volume</Trans>}
...
function TVLChartSection({ chainId }: { chainId: number }) {
  ...
  if (isSmallScreen) {
    ...
    return <MinimalStatDisplay title={<Trans>DeFi Pool TVL</Trans>} value={currentTVL} />
  }

  return (
    <SectionContainer>
      <SectionTitle>
        <Trans>DeFi Pool TVL</Trans>
      </SectionTitle>
      {(() => {
        if (dataQuality === DataQuality.INVALID) {
          const errorText = loading ? undefined : (
