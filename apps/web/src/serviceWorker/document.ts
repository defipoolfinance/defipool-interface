import { isAppDexOrg, isAppDexStagingOrg } from 'utils/env'
...
const DOCUMENT = process.env.PUBLIC_URL + '/index'
...
export function matchDocument({ request, url }: RouteMatchCallbackOptions) {
  ...
  if (!(isDevelopment() || isAppDexStagingOrg(url) || isAppDexOrg(url))) {
    return false
  }
