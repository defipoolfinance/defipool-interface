...
interface SwapAllowanceRequired {
  state: AllowanceState.REQUIRED
  token: Token
  isApprovalLoading: boolean
  isApprovalPending: boolean
...
export function useSwapAllowance({
  ...
  // if (!chainId) throw new Error('Must be chain Id at this point')

  // if (!chainId) throw new Error('Must be chainId at this point')

  const { tokenAllowance, isSyncing: isApprovalSyncing } = useTokenAllowance(token, account, routerAddress)
  const updateTokenAllowance = useUpdateTokenAllowance(amount, routerAddress)
  const revokeTokenAllowance = useRevokeTokenAllowance(token, routerAddress)
  ...
  const isApprovalPending = useHasPendingApproval(token, routerAddress)
  const isRevocationPending = useHasPendingRevocation(token, routerAddress)
  ...
  }, [approve, isApprovalLoading, isApprovalPending, isApproved, revoke, isRevocationPending, token, tokenAllowance])
}
/**
 * @deprecated permit-2
 */
