...
import { TGetSwapResponse } from 'rest/kyberswap/types-kyberswap'
import { PARASWAP_FEE_BPS, ParaswapSupportedChainId, paraswapFeeAddresses } from 'rest/paraswap/api-paraswap'
...
export enum TradeFillType {
  Classic = 'classic', // DeFi Pool V1, V2, and V3 trades with on-chain routes
...
export class ParaswapTrade {
  public readonly data: OptimalRate // TGetSwapResponse['data']['routeSummary']
  public readonly fillType = TradeFillType.Paraswap
  public readonly quoteMethod = QuoteMethod.QUICK_ROUTE // MAYBE NEW QUOTE METHOD ???
  public readonly tradeType: TradeType
  public readonly currencyAmountIn: CurrencyAmount<Currency>
  public readonly currencyAmountOut: CurrencyAmount<Currency>
  public readonly requestId = undefined
  public readonly gasUseEstimateUSD?: number
  public readonly priceImpact = new Percent(0)
  public readonly approveInfo: ApproveInfo = {
    needsApprove: false, // look to apps/web/src/state/routing/gas.ts
  }
  public readonly swapFee?: SwapFeeInfo
  ...
  constructor({
    ...
    this.data = response
    this.routerAddress = response.tokenTransferProxy // FIXME: may be wrong address
    const currencyAmountInApi = tryParseCurrencyAmount(
      formatUnits(this.data.srcAmount, currencyIn.decimals).toString(),
      currencyIn
    )
    if (!currencyAmountInApi) throw new Error('Failed to parse CurrencyAmount KyberswapTrade amountIn')

    // const currencyAmountOutApi = tryParseCurrencyAmount(formatEther(this.data.amountOut).toString(), currencyOut)
    const currencyAmountOutApi = tryParseCurrencyAmount(
      formatUnits(this.data.destAmount, currencyOut.decimals).toString(),
      currencyOut
    )
    if (!currencyAmountOutApi) throw new Error('Failed to parse CurrencyAmount KyberswapTrade amountOut')

    this.tradeType = tradeType
    // this.currencyAmountIn = tradeType === TradeType.EXACT_INPUT ? currencyAmountInApi : currencyAmountOutApi
    // this.currencyAmountOut = tradeType === TradeType.EXACT_INPUT ? currencyAmountOutApi : currencyAmountInApi
    this.currencyAmountIn = currencyAmountInApi
    this.currencyAmountOut = currencyAmountOutApi

    const feePercent = new Percent(PARASWAP_FEE_BPS, 10_000)
    // const feeCurrency =
    //   this.data.extraFee.chargeFeeBy === 'currency_out' ? this.currencyAmountOut : this.currencyAmountIn

    // this.swapFee = undefined

    this.swapFee = {
      /* raw amount of output token */
      amount:
        tradeType === TradeType.EXACT_INPUT
          ? feePercent.multiply(this.currencyAmountOut.quotient).quotient.toString()
          : feePercent.multiply(this.currencyAmountIn.quotient).quotient.toString(),

      recipient: paraswapFeeAddresses[this.currencyAmountIn.currency.chainId as ParaswapSupportedChainId], // this.data.extraFee.feeReceiver,
      percent: feePercent, // change it in paraswapConfirmSwap to.
    }

    this.gasUseEstimateUSD = parseFloat(this.data.gasCostUSD)
  }

  public get outputAmount(): CurrencyAmount<Token | Currency> {
    return this.currencyAmountOut
  }

  public get inputAmount(): CurrencyAmount<Token | Currency> {
    return this.currencyAmountIn
  }
...
// eslint-disable-next-line
export class KyberswapTrade {
  public readonly summary: TGetSwapResponse['data']['routeSummary']
  public readonly fillType = TradeFillType.Kyberswap
  public readonly quoteMethod = QuoteMethod.QUICK_ROUTE // MAYBE NEW QUOTE METHOD ???
  public readonly tradeType: TradeType
  public readonly currencyAmountIn: CurrencyAmount<Currency>
  public readonly currencyAmountOut: CurrencyAmount<Currency>
  ...
  constructor({
    ...
    // console.log(response)
    this.summary = response.data.routeSummary
    this.routerAddress = response.data.routerAddress
    const currencyAmountInApi = tryParseCurrencyAmount(
      formatUnits(this.summary.amountIn, currencyIn.decimals).toString(),
      currencyIn
    )
    if (!currencyAmountInApi) throw new Error('Failed to parse CurrencyAmount KyberswapTrade amountIn')

    // const currencyAmountOutApi = tryParseCurrencyAmount(formatEther(this.summary.amountOut).toString(), currencyOut)
    const currencyAmountOutApi = tryParseCurrencyAmount(
      formatUnits(this.summary.amountOut, currencyOut.decimals).toString(),
      currencyOut
    )
    if (!currencyAmountOutApi) throw new Error('Failed to parse CurrencyAmount KyberswapTrade amountOut')
    ...
    const feePercent = new Percent(this.summary.extraFee.isInBps ? this.summary.extraFee.feeAmount : 0, 10_000)
    const feeCurrency =
      this.summary.extraFee.chargeFeeBy === 'currency_out' ? this.currencyAmountOut : this.currencyAmountIn

    this.swapFee = {
      /* raw amount of output token */
      amount: feePercent.multiply(feeCurrency.quotient).quotient.toString(), // d.extraFee.feeAmount, TODO: calculate raw amount of output token

      recipient: this.summary.extraFee.feeReceiver,
      percent: feePercent,
      // new Percent(10, 10_000), // TODO: calculate fee using chargeFeeBy, isInBps, feeAmount, amountIn, amountOut
    }

    this.gasUseEstimateUSD = parseFloat(this.summary.gasUsd)
  }
...
export type SubmittableTrade = ClassicTrade | DutchOrderTrade | LimitOrderTrade | KyberswapTrade | ParaswapTrade
export type InterfaceTrade = SubmittableTrade | PreviewTrade
export type OperableTrade = ClassicTrade | KyberswapTrade | ParaswapTrade
...
export type TradeSource<T extends OperableTrade> = {
  state: TradeState
  trade?: T
  method?: QuoteMethod
