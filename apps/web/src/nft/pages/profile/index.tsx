...
const LoadedAccountPage = styled.div<{ cartExpanded: boolean; isListingNfts: boolean }>`
  ...
  margin: 0 ${({ isListingNfts }) => (isListingNfts ? LIST_PAGE_MARGIN : 0)}px;

  @media screen and (max-width: ${BREAKPOINTS.sm}px) {
    width: calc(100% - ${({ isListingNfts }) => (isListingNfts ? LIST_PAGE_MARGIN_MOBILE * 2 : 0)}px);
    margin: 0 ${({ isListingNfts }) => (isListingNfts ? LIST_PAGE_MARGIN_MOBILE : 0)}px;
  }
`
...
function getProfilePageTitle(account: string | undefined, ENSName: string | null | undefined): string {
  if (!account) {
    return t`NFT collection on DeFi Pool`
  }

  if (!ENSName) {
    return t`NFT collection on DeFi Pool - ${shortenAddress(account)}`
  }

  return t`${ENSName}'s NFT collection on DeFi Pool`
}
