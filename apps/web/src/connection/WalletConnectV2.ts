  ...
  constructor({
    ...
    super({
      ...
      options: {
        ...
        metadata: {
          name: 'DeFi Pool',
          description: 'DeFi Pool',
          url: 'https://pxp-dex.itproject.dev',
          icons: ['https://pxp-dex.itproject.dev/favicon.png'],
        },
...
// Custom class for DeFi Pool Wallet specific functionality
export class UniwalletConnect extends WalletConnectV2 {
  ANALYTICS_EVENT = 'DeFi Pool Wallet QR Scan'
  ...
  constructor({ actions, onError }: Omit<WalletConnectConstructorArgs, 'options'>) {
    ...
    this.events.on(URI_AVAILABLE, (uri) => {
      ...
      // Emits custom wallet connect code, parseable by the DeFi Pool Wallet
      this.events.emit(UniwalletConnect.UNI_URI_AVAILABLE, `https://uniswap.org/app/wc?uri=${uri}`)

      // Opens deeplink to DeFi Pool Wallet if on iOS
      if (isIOS || isAndroid) {
