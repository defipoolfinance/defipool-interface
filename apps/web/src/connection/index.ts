...
// !getIsMetaMaskWallet() && (!getIsInjected() || getIsCoinbaseWallet())
...
export const deprecatedInjectedConnection: Connection = {
  ...
  overrideActivate: () => {
    if (getShouldAdvertiseMetaMask()) {
      ...
      const domainName = config.clientWebUrl.replace('https://', '')
      window.open(`https://metamask.app.link/dapp/${domainName}`, 'open_metamask') // 'inst_metamask')
      return true
    }
    return false
  },
...
export const uniwalletWCV2ConnectConnection: Connection = {
  getProviderInfo: () => ({ name: 'DeFi Pool Wallet', icon: UNIWALLET_ICON }),
